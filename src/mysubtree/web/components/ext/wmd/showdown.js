...
Attacklab.showdown.converter = function () {
	...
	this.makeHtml = function (text) {
	    ...
	    text = text.replace(/https?\:\/\/[^"\s\<\>]*[^.,;'">\:\s\<\>\)\]\!]/g, function (wholeMatch, matchIndex){
			...
			return "<a href=\"" + wholeMatch + "\" rel=\"nofollow\">" + wholeMatch + "</a>"; //                                                               CHANGED: ' to "       +        ADDED rel="nofollow"
		});
...
/*
	...
	*/
	...
	var writeAnchorTag = function (wholeMatch, m1, m2, m3, m4, m5, m6, m7) {
		...
		result += " rel=\"nofollow\">" + link_text + "</a>"; //                                                                                  ADDED rel="nofollow"
...
// 		text = text.replace(/>/g, "&gt;"); //                                                                                 COMMENTED
		...
		// Encode "smart" quotes
// 		text = text.replace(/‘/g, "&lsquo;"); //                                                                                 COMMENTED
// 		text = text.replace(/’/g, "&rsquo;"); //                                                                                 COMMENTED
// 		text = text.replace(/“/g, "&ldquo;"); //                                                                                 COMMENTED
// 		text = text.replace(/”/g, "&rdquo;"); //                                                                                 COMMENTED
// 		text = text.replace(/–/g, "&mdash;"); //                                                                                 COMMENTED
	...
	var _DoItalicsAndBold = function (text) {
		...
		//text = text.replace(/(\*\*|__)(?=\S)([^\r]*?\S[*_]*)\1/g, "<strong>$2</strong>");
                text = text.replace(/(\*\*)(?=\S)([^\r]*?\S[*]*)\1/g, "<strong>$2</strong>"); // disable __bold__                       CHANGED: commented 1 line above; added this line

// 		text = text.replace(/(\w)_(\w)/g, "$1~E95E$2") // ** GFM **  "~E95E" == escaped "_"
// 		text = text.replace(/(\*|_)(?=\S)([^\r]*?\S)\1/g, "<em>$2</em>");
                text = text.replace(/(\*)(?=\S)([^\r]*?\S)\1/g, "<em>$2</em>"); // disable _italics_                                    CHANGED: commented 2 lines above; added this line
...
/*
	...
	*/
		...
		text = text.replace(/((^[ \t]*&gt;[ \t]?.+\n(.+\n)*\n*)+)/gm, function (wholeMatch, m1) { //                             CHANGED: > to &gt;
			var bq = m1;

			// attacklab: hack around Konqueror 3.5.4 bug:
			// "----------bug".replace(/^-/g,"") == "bug"
			bq = bq.replace(/^[ \t]*&gt;[ \t]?/gm, "~0"); // trim one level of quoting                                       CHANGED: > to &gt;
			...
			bq = bq.replace(/(\s*<pre class="prettyprint">[^\r]+?<\/pre>)/gm, function (wholeMatch, m1) { //                                     CHANGED: added class attribute
				var pre = m1;
				// attacklab: hack around Konqueror 3.5.4 bug:
	...
	var _DoAutoLinks = function (text) {
		...
		text = text.replace(/(?:")<((https?|ftp|dict):[^'">\s]+)>/gi, "<a href=\"$1\" rel=\"nofollow\">$1</a>"); //                                                                 ADDED rel="nofollow"
