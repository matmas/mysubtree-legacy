...
(function () {
	...
	WMDEditor.defaults = { // {{{
		...
		helpLink: "/en/editing-help",
		helpHoverTitle: "Editing help",
		helpTarget: "_blank",
		...
		tagFilter: {
			enabled: true,
			allowedTags: /^(<\/?(b|blockquote|code|del|dd|dl|dt|em|h1|h2|h3|i|kbd|li|ol|p|pre|pre\sclass="prettyprint"|s|sup|sub|strong|strike|ul)>|<(br|hr)\s?\/?>)$/i, //                     ADDED |pre\sclass="prettyprint"
                      //patternLink: /^(<a\shref="(\#\d+|(https?|ftp):\/\/[-A-Za-z0-9+&@#\/%?=~_|!:,.;\(\)]+)"(\stitle="[^"<>]+")?\s?>|<\/a>)$/i,
                        patternLink: /^(<a\shref="(\#\d+|(https?|ftp):\/\/[-A-Za-z0-9+&@#\/%?=~_|!:,.;\(\)]+)"(\stitle="[^"<>]+")?\srel="nofollow"\s?>|<\/a>)$/i, //                                        CHANGED to rel="nofollow"
	...
	var PreviewManager = function (wmd) { // {{{
		...
		var makePreviewHtml = function () {
			...
			if (converter) {
				text = converter.makeHtml(text.split("&").join("&amp;").split( "<").join("&lt;").split(">").join("&gt;"));
			}
		...
		var pushPreviewHtml = function (text) {
			...
			if (wmd.panels.output) {
				...
				// Send the HTML wrapped in <pre><code>
					...
					wmd.panels.output.innerHTML = "<pre class=\"prettyprint\"><code>" + newText + "</code></pre>"; //                                               !!! CHANGED !!!       (added class attribute)
				}
			}
